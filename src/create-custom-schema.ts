...
import storedData from './stored.json'
export async function createCustomSchema(
  ...
  const aliceKeys = await generateKeypairs(storedData.aliceMnemonic)
  // console.log(aliceMnemonic,aliceDid)
  const schema = Cord.Schema.fromProperties(
    schemaTitle,
    fieldsAsJsonObject,
    storedData.aliceDidUri
  )
  try {
